...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
	try {
		...
		if (!parsedUserId) {
			return badRequest('userId is required and must be a valid positive number')
		}

		let body: userFormSchemaType
		try {
		...
		const response = await fetch(`${BASE_URL}/users/${parsedUserId}`, {
			method: 'PATCH',
